...
function [GQKSA] = Quantize_Projections_VT_v2a(tro,T,qT,KSA)
...
if nproj == 1
    for p = 1:3
        GQKSA(1,:,p) = interp1(realt(1,:),KSA(1,:,p),qT,'spline','extrap');
    end
else  
    for n = 1:nproj
        for p = 1:3
            GQKSA(n,:,p) = interp1(realt(n,:),KSA(n,:,p),qT,'spline','extrap');
        end
    end
end
