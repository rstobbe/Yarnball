%==================================================================
% (v2c)
%       - Matching 'YarnBallOutRphsSingleEcho_v2c'
%==================================================================

classdef DesType_YarnBallOutInDualEcho_v2c < handle

properties (SetAccess = private)                   
    Method = 'DesType_YarnBallOutInDualEcho_v2c'
    Name
    Panel
    DESTYPEipt
    TurnEvolutionFunc
...
% Constructor
%==================================================================  
function DESTYPE = DesType_YarnBallOutInDualEcho_v2c(DESTYPEipt)

    %---------------------------------------------
    % Load Panel Input
    %---------------------------------------------
    DESTYPE.DESTYPEipt = DESTYPEipt;
    DESTYPE.OutFov = str2double(DESTYPEipt.('FoV'));
    DESTYPE.OutVox = str2double(DESTYPEipt.('Vox'));
    DESTYPE.OutTro = str2double(DESTYPEipt.('Tro'));
    DESTYPE.InFov = DESTYPE.OutFov;
    DESTYPE.InVox = DESTYPE.OutVox;
    DESTYPE.InTro = DESTYPE.OutTro;    
    DESTYPE.OutTurnSlope = str2double(DESTYPEipt.('TurnSlope'));
    DESTYPE.OutTurnEnd = str2double(DESTYPEipt.('TurnEnd'));
    DESTYPE.InTurnSlope = DESTYPE.OutTurnSlope;
    DESTYPE.InTurnEnd = DESTYPE.OutTurnEnd;
...
function Initialize(DESTYPE,SYS,NUC)
    ...
    %   (Out/In the same)
    %---------------------------------------------
    DESTYPE.KINFO(1).SetFov(DESTYPE.OutFov);
    DESTYPE.KINFO(1).SetVox(DESTYPE.OutVox);
    DESTYPE.KINFO(1).SetTro(DESTYPE.OutTro);
    DESTYPE.KINFO(1).SetElip(DESTYPE.ELIP.Elip);
    DESTYPE.KINFO(1).SetYbAxisElip(DESTYPE.ELIP.YbAxisElip);
    DESTYPE.KINFO(1).SetDirOut;
    DESTYPE.KINFO(2).SetFov(DESTYPE.OutFov);
    DESTYPE.KINFO(2).SetVox(DESTYPE.OutVox);
    DESTYPE.KINFO(2).SetTro(DESTYPE.OutTro);
    DESTYPE.KINFO(2).SetElip(DESTYPE.ELIP.Elip);
    DESTYPE.KINFO(2).SetYbAxisElip(DESTYPE.ELIP.YbAxisElip);
    DESTYPE.KINFO(2).SetDirIn;
    ...
    DESTYPE.GENPRJ.SetkMax(DESTYPE.KINFO(1).kmax);
    DESTYPE.GENPRJ.SetGradTimeQuant(DESTYPE.GradTimeQuant);
    DESTYPE.GENPRJ.SetTro(DESTYPE.KINFO(1).tro);
    DESTYPE.GENPRJ.SetElip(ELIPfunc,ELIPipt);
    DESTYPE.GENPRJ.SetSpin(SPINfunc,SPINipt);
    DESTYPE.GENPRJ.SPIN.SetMatRad(DESTYPE.KINFO(1).rad);
    ...
    DESTYPE.GENPRJ(2).SetkMax(DESTYPE.KINFO(2).kmax);
    DESTYPE.GENPRJ(2).SetGradTimeQuant(DESTYPE.GradTimeQuant);
    DESTYPE.GENPRJ(2).SetTro(DESTYPE.KINFO(2).tro);
    DESTYPE.GENPRJ(2).SetElip(ELIPfunc,ELIPipt);
    DESTYPE.GENPRJ(2).SetSpin(SPINfunc,SPINipt);
    DESTYPE.GENPRJ(2).SPIN.SetMatRad(DESTYPE.KINFO(2).rad);
    DESTYPE.GENPRJ(2).SPIN.DefineSpin;
    DESTYPE.GENPRJ(2).SetTurnEvo(TURNEVOfunc,TURNEVOipt);
    DESTYPE.GENPRJ(2).TURNEVO.SetSlope(DESTYPE.InTurnSlope);
    DESTYPE.GENPRJ(2).TURNEVO.SetEnd(DESTYPE.InTurnEnd);
    ...
    DESTYPE.KINFO(2).SetNproj(DESTYPE.GENPRJ(2).SPIN.NumProj);
    DESTYPE.KINFO(1).SetDesignTimingStart(0);
    DESTYPE.KINFO(2).SetDesignTimingStart(DESTYPE.KINFO(1).tro + DESTYPE.GradTimeQuant);
    DESTYPE.SamplingTro = DESTYPE.OutTro + DESTYPE.GradTimeQuant + DESTYPE.InTro;
...
function rkSpaceOut = Build(DESTYPE,InitYB)
    ...
    rkSpaceOut0 = cat(2,rkSpace1,flip(rkSpace2,2));
    rkSpaceOut = DESTYPE.ORNT.Orient(rkSpaceOut0,DESTYPE.SYS,DESTYPE.GENPRJ(1).ELIP,DESTYPE.KINFO(1));
    DESTYPE.ORNT.Orient(rkSpaceOut0,DESTYPE.SYS,DESTYPE.GENPRJ(2).ELIP,DESTYPE.KINFO(2));               % to write changes into KINFO(2)
end    
...
function Test(DESTYPE)
    ...
    stro = num2str(10*DESTYPE.KINFO(1).tro,'%03.0f');
    snproj = num2str(DESTYPE.KINFO(1).nproj,'%4.0f');
    DESTYPE.Name = ['DES_F',sfov,'_V',svox,'_E',selip,'_T',stro,'_N',snproj,'_S',DESTYPE.GENPRJ(1).SPIN.name,'_DEOI']; 
    ...
    Panel0(8,:) = {'Tro (ms)',DESTYPE.KINFO(1).tro,'Output'};
