%==================================================================
% (v2c)
%       - Matching 'YarnBallMultiEchoInOutRephase_v2c'
%==================================================================

classdef DesType_YarnBallMultiEchoInOutRephase_v2c < handle

properties (SetAccess = private)                   
    Method = 'DesType_YarnBallMultiEchoInOutRephase_v2c'
    Name
    Panel
    DESTYPEipt
    TurnEvolutionFunc
...
% Constructor
%==================================================================  
function DESTYPE = DesType_YarnBallMultiEchoInOutRephase_v2c(DESTYPEipt)
...
function Initialize(DESTYPE,SYS,NUC)
    ...
    %---------------------------------------------
    for n = 1:DESTYPE.NumEchos
        DESTYPE.KINFO(n).SetNproj(DESTYPE.GENPRJ(1).SPIN.NumProj/2);
        DESTYPE.KINFO(n).SetDesignSamplingTimeStart(DESTYPE.RphsTro + DESTYPE.GradTimeQuant + (n-1)*2*(DESTYPE.Tro + DESTYPE.GradTimeQuant));
        DESTYPE.KINFO(n).SetDesignSamplingTimeToCentre(DESTYPE.RphsTro + DESTYPE.GradTimeQuant + DESTYPE.Tro + DESTYPE.GradTimeQuant/2 + (n-1)*2*(DESTYPE.Tro + DESTYPE.GradTimeQuant));
        DESTYPE.KINFO(n).SetDesignTro(2*DESTYPE.Tro + DESTYPE.GradTimeQuant);
    end
    DESTYPE.SamplingTro = DESTYPE.NumEchos*2*(DESTYPE.Tro + DESTYPE.GradTimeQuant) + 2*DESTYPE.RphsTro + DESTYPE.GradTimeQuant;
...
function rkSpaceOut = Build(DESTYPE,InitYB)
    ...
    rkSpaceOut0 = cat(2,rkSpace1);
    for n = 1:DESTYPE.NumEchos
        if rem(n,2)
            rkSpaceOut0 = cat(2,rkSpaceOut0,flip(rkSpace2,2),-rkSpace2);
        else
            rkSpaceOut0 = cat(2,rkSpaceOut0,-flip(rkSpace2,2),rkSpace2);
        end
    end
    if rem(n,2)
        rkSpaceOut0 = cat(2,rkSpaceOut0,flip(-rkSpace1,2));
    else
        rkSpaceOut0 = cat(2,rkSpaceOut0,flip(rkSpace1,2));
    end
...
function Test(DESTYPE)
    ...
    DESTYPE.Name = ['DES_F',sfov,'_V',svox,'_E',selip,'_T',stro,'_N',snproj,'_S',DESTYPE.GENPRJ(2).SPIN.name,'_',sechos,'IOR']; 
