%==================================================================
% (v2f)
%       - Matching 'YarnBallMultiEchoInOut_v2f'
%==================================================================

classdef DesType_YarnBallMultiEchoOutOut_v2f < handle

properties (SetAccess = private)                   
    Method = 'YarnBallMultiEchoOutOut_v2f'
    Name
    Panel
    DESTYPEipt
    TurnEvolutionFunc
...
% Constructor
%==================================================================  
function DESTYPE = DesType_YarnBallMultiEchoOutOut_v2f(DESTYPEipt)
    ...
    DESTYPE.EndDel = str2double(DESTYPEipt.('EndDel'));
...
function Initialize(DESTYPE,SYS,NUC)
    ...
    for n = 1:DESTYPE.NumEchos
        ...
        DESTYPE.KINFO(n).SetDesignSamplingTimeStart((n-1)*(DESTYPE.Tro + DESTYPE.RphsTro + (2+DESTYPE.EndDel)*DESTYPE.GradTimeQuant));
        DESTYPE.KINFO(n).SetDesignSamplingTimeToCentre((n-1)*(DESTYPE.Tro + DESTYPE.RphsTro + (2+DESTYPE.EndDel)*DESTYPE.GradTimeQuant));
        DESTYPE.KINFO(n).SetDesignTro(DESTYPE.Tro);
    end
    DESTYPE.SamplingTro = DESTYPE.NumEchos*(DESTYPE.Tro + DESTYPE.RphsTro + (2+DESTYPE.EndDel)*DESTYPE.GradTimeQuant) - DESTYPE.GradTimeQuant;
...
function rkSpaceOut = Build(DESTYPE,InitYB)
    ...
    sz = size(InitYB);
    EndZero = zeros(sz(1),DESTYPE.EndDel,3);
    rkSpaceOut0 = [];
    for n = 1:DESTYPE.NumEchos
        rkSpaceOut0 = cat(2,rkSpaceOut0,rkSpace1,flip(rkSpace2,2),EndZero);
    end
