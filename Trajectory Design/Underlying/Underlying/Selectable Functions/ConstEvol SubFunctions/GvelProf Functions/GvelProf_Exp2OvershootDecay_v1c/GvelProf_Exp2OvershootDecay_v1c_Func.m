...
GVP.Accprof = ['@(Acc0,AccMax,t) Acc0+(AccMax-((1./(1+exp(',num2str(GVP.decayrate),'*(-t+',num2str(GVP.decayshift),'))))*AccMax*',num2str(GVP.startfrac-1),')-heaviside(t-',num2str(GVP.decayshift),').*AccMax.*',num2str(GVP.enddrop),'.*(t-',num2str(GVP.decayshift),')-Acc0).*(1 - exp(-t/',num2str(GVP.tau),'))'];
